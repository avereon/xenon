...
* **Internationalization** - Xenon supports internationalization and localization
  through the use of resource bundles and locales. Module developers are encouraged
  to use good internationalization and localization practices.
* **Resources** - Xenon provides features to manage resources (e.g. resources, files,
  connections). In general, resources are defined by URI and then Xenon takes care
  of managing the resource life cycle from beginning to end.
* **Tools** - Xenon provides the base classes for creating tools. Tools are used to
  view and modify resources. Even Xenon uses tools for the program resources like the
  about page and settings.
* **Settings** - Xenon provides features for managing program, module, tool and resource
  settings. There are already several predefined setting types, and custom types
  can be created.
* **Modules** - Xenon provides features for managing modules (a.k.a. plugins, extensions,
  modules, etc.). Xenon is nothing more than a framework and modules provide the
  desired functionality. Module developers can also set up module repositories to host
  their modules for their users.
* **Updates** - Xenon provides functionality for handling updates when available.
  Xenon looks for both framework and module updates from one or more repositories.
...
* **Icons** - Xenon provides many predefined icons and provides developers with APIs
  ...
  can be registered and used from modules.
* **Actions** - Xenon provides many predefined actions for use in menus and
  toolbars. Custom actions can be registered and used from modules.
...
* **Search and Index** - Xenon provides common search and index functionality. 
  Module developers are encouraged to provide indexed content for users to find 
